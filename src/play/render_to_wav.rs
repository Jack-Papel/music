...
#![allow(clippy::arithmetic_side_effects, clippy::cast_possible_truncation, clippy::cast_possible_wrap, clippy::cast_precision_loss, clippy::cast_sign_loss, reason = "Complex audio processing code")]
...
impl MusicPlayer<FileOutputConfig> {
    ...
    pub fn render_to_wav<T: Playable + Clone + Send + Sync + 'static>(&self, piece: T, path: &str) {
        ...
        let total_samples: usize = (sample_rate as u64).saturating_mul(total_ms).div(1000).try_into().unwrap_or(usize::MAX);
            ...
            for note in notes {
                match note.1 {
                    crate::note::NoteKind::Pitched { pitch, timbre, volume } => {
                        ...
                        let native_samples = (native_sample_rate as u64).saturating_mul(duration_ms).div(1000).try_into().unwrap_or(usize::MAX);
                        ...
                        for in_ch in 0..native_channels {
                            ...
                            let out_ch = if native_channels == 1 {
                                ...
                                let idx = ((in_ch as f32) * (max_channels as f32 - 1.0) / (native_channels as f32 - 1.0)).round() as usize;
                                vec![idx]
                            };
                            let buf = if sample_rate != native_sample_rate {
                                ...
                                resample_to_target_rate(chans[in_ch].clone().into_iter(), native_sample_rate, sample_rate, note_samples)
                            } else {
                                chans[in_ch].clone()
                            };
                            // Append all the samples to the output channels
                            let start_idx = (sample_rate as u64).saturating_mul(start_ms).div(1000).try_into().unwrap_or(usize::MAX);
                                ...
                                if let Some(idx) = start_idx.checked_add(i) {
                                    for &ch in &out_ch {
                                        if idx < samples[ch].len() {
                                            ...
                                            let val = if native_channels == 1 { s / max_channels as f32 } else { s };
                                            samples[ch][idx] += val;
                                        }
                                    }
                                }
...
#[expect(clippy::cast_possible_truncation, clippy::cast_sign_loss, clippy::cast_precision_loss, clippy::arithmetic_side_effects, clippy::cast_possible_wrap, reason = "Cubic interpolation and resampling require these conversions; safe for audio.")]
// I assume this approximates inbetweening the samples using the cubic interpolation.
fn resample_to_target_rate<I: Iterator<Item = f32>>(input: I, input_rate: u32, output_rate: u32, num_samples: usize) -> Vec<f32> {
    ...
    for i in 0..num_samples {
        ...
        let y2 = *input.get((idx + 1).min((input_len - 1) as isize) as usize).unwrap_or(&0.0);
        let y3 = *input.get((idx + 2).min((input_len - 1) as isize) as usize).unwrap_or(&0.0);
